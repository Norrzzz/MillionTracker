try {
    ...
    $Headers = @{ "Accept" = "application/json"; "X-CMC_PRO_API_KEY" = $cmcKey }
    ...
    mysql -e "UPDATE MillionTracker.ranks SET coinmarketcap = $CMCRank, nomics = $NomicsRank, coingecko = $CoinGeckoRank, dextools = $DEXToolsRank WHERE id = 1;" -u cronuser --password=$env:CRONUSER_PWD
